...
function TWEAK_THE_SHIT{
    ...
    if ($CheckBox62.Checked){
        if ($Language -eq "de-DE"){
            $Backup = [System.Windows.Forms.MessageBox]::Show("Ein Backup der Registry wird ausgef�hrt. Eine Normale Windows Registry ist im Normalfall ca. 500mb Gro�. Das Backup wird unter C:\RegBack\ Gespeichert.","TGF Tuning Pack 4.2 by MinersWin",'OK','Info')
        } else {
    ...
    if ($Language -eq "de-DE"){
        ...
        Write-Host "Unterst�zung dieses und weiteren Tools: https://paypal.me/minerswin" -ForegroundColor Green
...
function Installer-Cleanup{
    [System.Windows.Forms.MessageBox]::Show("Diese funktion ist momentan nicht verf�gbar, Microsoft hat dieses Feature eingestellt. Ich arbeite momentan an einer Alternative","TGF Tuning Pack 4.1 by MinersWin",1)
}
#System File Checker
$Button12.Add_Click{(System-File-Checker)}
function System-File-Checker{
    [System.Windows.Forms.MessageBox]::Show("Es wird sich gleich ein Konsolenfenster �ffnen, in welchem ein Windows Image heruntergeladen wird, mit welchem das System auf Fehler gepr�ft wird. Das Image wird nach Abschluss des Vorgangs wieder gel�scht. Zum Best�tigen OK dr�cken","TGF Tuning Pack 4.1 by MinersWin",1)    
